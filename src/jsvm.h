...
/**
 * @brief This API can be called even if there is a pending JavaScript exception.
 *
 * @param env: The environment that the API is invoked under.
 * @param code: Optional JSVM_Value with the string for the error code to be associated with the error.
 * @param msg: JSVM_Value that references a JavaScript string to be used as the message for the Error.
 ...
 * @param initial_refcount: Initial reference count for the new reference.
 ...
 * data through JavaScript code, so it can be retrieved later by native code using JSVM_get_value_external.
 ...
 * a distinct value type: calling JSVM_typeof() with an external value yields JSVM_external.
 *
 * @param env: The environment that the API is invoked under.
...
/**
 * @brief This API returns a JSVM-API value corresponding to a JavaScript ArrayBuffer.
 ...
 *
 * @param env: The environment that the API is invoked under.
 * @param externalData: Pointer to the underlying byte buffer of the ArrayBuffer.
 * @param byteLength: The length in bytes of the underlying buffer.
 * @param finalizeCb: Optional callback to call when the ArrayBuffer is being collected.
 * JSVM_finalize provides more details.
 * @param finalizeHint: Optional hint to pass to the finalize callback during collection.
 * @param result: A JSVM_value representing a JavaScript ArrayBuffer.
 * @return Returns JSVM_ok if the API succeeded.
 * @since 11
 */
JSVM_EXTERN JSVM_Status OH_JSVM_CreateExternalArraybuffer(JSVM_Env env,
                                                          void* externalData,
                                                          size_t byteLength,
                                                          JSVM_Finalize finalizeCb,
                                                          void* finalizeHint,
 ...
 * of elements in the array. sign_bit and words may be both set to NULL, in order to get only word_count.
 *
 * @param env: The environment that the API is invoked under.
 * @param value: JSVM_value representing JavaScript BigInt.
 ...
 * @param env: The environment that the API is invoked under.
 * @param object: The object from which to retrieve the properties.
 * @param key_mode: Whether to retrieve prototype properties as well.
 * @param key_filter: Which properties to retrieve (enumerable/readable/writable).
 * @param key_conversion: Whether to convert numbered property keys to strings.
 ...
 * JOH_JSVM_GetElement can be used to iterate over result.
 * @return Returns JSVM_ok if the API succeeded.
 * @since 11
 */
 ...
 * object is invoked. JSVM_callback provides more details.
 * @param data: User-provided data context. This will be passed back into the
 ...
 * signature. A C++ class constructor cannot be used. JSVM_callback provides more details.
 * @param data: Optional data to be passed to the constructor callback as the data
 ...
 * properties, accessors, and methods on the class See JSVM_property_descriptor.
 ...
 * be retrieved later using JSVM_unwrap().
 *
 * @param env: The environment that the API is invoked under.
 ...
 * the call can be obtained then by calling JSVM_unwrap() on the wrapper object.
 *
 * @param env: The environment that the API is invoked under.
...
/**
 * @brief Retrieves a native instance that was previously wrapped in the JavaScript object js_object
 * using JSVM_wrap() and removes the wrapping. If a finalize callback was associated with the wrapping,
...
/**
 * @brief Associates the value of the type_tag pointer with the JavaScript object or external.
 * JSVM_check_object_type_tag() can then be used to compare the tag that was attached to the
...
/**
 * @brief Compares the pointer given as type_tag with any that can be found on js_object.
 * If no tag is found on js_object or, if a tag is found but it does not match type_tag,
 * then result is set to false. If a tag is found and it matches type_tag, then result is set to true.
 *
 * @param env: The environment that the API is invoked under.
 ...
 * @param finalizeData: Optional data to be passed to finalize_cb.
 ...
 * @since 11
 */
JSVM_EXTERN JSVM_Status OH_JSVM_PostFinalizer(JSVM_Env env,
                                              JSVM_Finalize finalizeCb,
                                              void* finalizeData,
                                              void* finalizeHint);

/**
 * @brief This API returns the highest JSVM-API version supported by the JSVM.js runtime.
 *
 * JSVM-API is planned to be additive such that newer releases of JSVM.js may support additional
 * API functions. In order to allow an addon to use a newer function when running with versions
 * of JSVM.js that support it, while providing fallback behavior when running with JSVM.js
...
/**
 * @brief This API return the parse object of json string.
 * @param env: The environment that the API is invoked under.
...
/**
 * @brief This API return the string stringified from object.
 * @param env: The environment that the API is invoked under.
