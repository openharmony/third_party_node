...
function prepareStackTrace(error, trace) {
  try {
    const sourceMap = read('/data/bundle.js.map/bundle.js.map')
    const payload = JSON.parse(sourceMap)
    const sm = new SourceMap(payload)
    const preparedStack = trace.map((t, i) => {
      const str = i !== 0 ? '\n    at ' : '';
        const {
          originalLine,
          originalColumn,
          originalSource,
        } = sm.findEntry(t.getLineNumber() - 327, t.getColumnNumber() - 1);
        if (originalSource && originalLine !== undefined &&
          ...
          return `${str}${originalName}${hasName ? ' (' : ''}` +
...
OH_JSVM_CompileScriptWithOrigin(JSVM_Env env,
  ...
  v8::String::Utf8Value scriptV8Str(isolate, v8_script);
  std::string scriptString(*scriptV8Str);
  auto newScript = SourceMapRunner + scriptString;
  JSVM_Value sourceMapSource;
  OH_JSVM_CreateStringUtf8(env, newScript.c_str(), newScript.size(), &sourceMapSource);
  v8::Local<v8::Value> v8_script_new = v8impl::V8LocalValueFromJsValue(sourceMapSource);


  v8::Local<v8::Value> sourceMapUrl =
    v8::String::NewFromUtf8(isolate, origin->sourceMapUrl).ToLocalChecked();
  ...
  v8::ScriptCompiler::CachedData* cache = cachedData
    ? new v8::ScriptCompiler::CachedData(cachedData, cachedDataLength) : nullptr;
  v8::ScriptCompiler::Source scriptSource(v8_script_new.As<v8::String>(), scriptOrigin, cache);
  auto option = cache ? v8::ScriptCompiler::kConsumeCodeCache
    : v8::ScriptCompiler::kNoCompileOptions;
